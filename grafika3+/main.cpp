...
struct Camera {
	static const int XM = 720;
	static const int YM = 720;
...
struct Scene {
	...
	void addCsirguru() {
		...
		int randMax = CSIRGURU_FIELD_LIMIT - APPROX_JUMP_LENGTH;
		last->csirguru.position.x = rand() % (2 * randMax) - randMax;
		last->csirguru.position.z = rand() % (2 * randMax) - randMax;
	}
...
void onKeyboard(unsigned char key, int x, int y) {
	...
	case 'd': case 'D':
		...
		break;
	case 'r': case 'R':
		scene.bomb.position.x = scene.bomb.position.z = 0;
		scene.deleteDeadCsirgurus();
		break;
	case ' ':
...
int main(int argc, char **argv) {
	...
	glutInitWindowSize(720, 720);			// Alkalmazas ablak kezdeti merete 600x600 pixel 
